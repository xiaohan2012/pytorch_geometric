...
class Logger:
    ...
    def clustering(self):
        ...
        return {
            "ami":
            float(round(adjusted_mutual_info_score(true, pred), cfg.round)),
            "ari": float(round(adjusted_rand_score(true, pred), cfg.round)),
        }
    ...
    def write_epoch(self, cur_epoch):
        ...
        if not task_stats:  # use default metrics if no matching custom metric
            if self.task_type == "regression":
                ...
                task_stats = self.clustering()
            else:
                raise ValueError(
                    "Task has to be regression, classification, or clustering")
